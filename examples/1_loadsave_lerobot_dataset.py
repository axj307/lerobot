...
def prepare_episode_data(frames, states, actions, language_instruction="Pick up the cube", embedding_dim=512):
    ...
    for frame_idx in range(num_frames):
        # Convert torch tensor to numpy and normalize
        frame = frames[frame_idx].numpy()
        if frame.max() > 1.0:
            frame = frame / 255.0
        ...
        episode = {
            'observation': {
                'images': {   
                    'nexigo': frame.astype(np.float32),
                },
...
def save_complete_dataset(dataset, base_output_dir, ds_meta):
    ...
    episodes_dir = os.path.join(base_output_dir, 'episodes')
    os.makedirs(episodes_dir, exist_ok=True)
    
    # Save metadata using ds_meta instead of dataset.meta
    metadata = {
...
# Usage
# output_directory = "/mnt/data/amit/projects/ai/lerobot/output/datasets"
